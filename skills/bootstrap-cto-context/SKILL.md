...
3. If the project folder does not exist, offer to create it. Use `dzcto init "<project folder>"` when the wrapper is available, or `python3 scripts/dzcto.py init "<project folder>"` from this plugin repo, to create `<project>/knowledge/wiki/`, the standard folder structure, `knowledge/wiki/.dzcto/` sidecar metadata, and `knowledge/wiki/index.html`.
