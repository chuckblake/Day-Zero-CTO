...
- Generated Decisions and Risks pages include short regenerated Current Read summaries above the source logs. Do not hand-edit those summaries; update the source Markdown or structured report data, then refresh.
- Tech Stack, Engineering Risk, Weekly Review, and CEO Update report risks are candidate signals. They roll up to `core/risks.html#risk-signals`; promote actionable items into `core/RISKS.md` with source, owner, mitigation, and a calendar next review date.
