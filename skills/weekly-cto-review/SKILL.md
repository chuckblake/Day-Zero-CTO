...
description: "Run a recurring startup CTO review and create a durable HTML weekly review in the Day Zero CTO home folder. Use when the user asks for a weekly CTO review, engineering health check, startup operating review, leadership recap, next-week technical priorities, or a regular ceremony that summarizes delivery, risk, decisions, team/process health, and CEO-update material."
---
...
3. Load context files from the Day Zero CTO home if present: `core/STRATEGY.md`, `core/TEAM.md`, `core/OPERATING_CADENCE.md`, `core/DECISIONS.md`, `core/RISKS.md`, and recent reports.
...
7. Write the canonical review as an HTML artifact under `<Day Zero CTO home>/reports/weekly-reviews/` and regenerate `<Day Zero CTO home>/index.html`.
...
```bash
scripts/dzcto-artifact.rb --home "<Day Zero CTO home>" --kind weekly-reviews --title "Weekly CTO Review" --body-file "<html body file>"
```
...
- Preserve or update `core/RISKS.md` and `core/DECISIONS.md` only in the Day Zero CTO home, and only when the user asks for durable follow-through.
