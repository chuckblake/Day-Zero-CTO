...
- Treat `core/RISKS.md` as the active risk register and editable source of truth. Dashboard risk cards and `core/risks.html` are generated from it. Use the `review-risks` skill when walking risks one by one to keep, update, close, punt, or mark evidence needed. Every active risk must have a calendar `Next Review` date; external triggers may be included but cannot replace the date.
