...
SAMPLE_REPORT_STEM = "sample-ceo-report"
...
def is_sample_report(data: Any) -> bool:
    return isinstance(data, dict) and data.get("sample") is True


def is_sample_report_path(path: Path) -> bool:
    try:
        return is_sample_report(read_json_file(path.with_suffix(".json"), None))
...
def render_sample_report_banner() -> str:
    return """
<aside class="report-sample" aria-label="Sample report notice">
  <strong>Sample report</strong>
  <span>This is an example report generated by dzcto init to show the report format. It is excluded from report counts, the weekly streak, and prior-report comparison.</span>
</aside>
"""
...
def render_structured_report(
    ...
    if is_sample_report(data):
        rendered = f"{render_sample_report_banner()}{rendered}"
    return rendered
...
def build_sample_report_data(company: str, today: dt.date | None = None) -> dict[str, Any]:
    today = today or dt.date.today()
    end = today - dt.timedelta(days=1)
    start = end - dt.timedelta(days=6)
    return {
        "schema_version": CEO_REPORT_SCHEMA_VERSION,
        "report_type": "weekly",
        "sample": True,
        "company": company,
        "window": {"start": start.isoformat(), "end": end.isoformat()},
        "headline": "Example report generated by dzcto init; this is illustrative content, not real engineering evidence.",
        "progress": [
            {
                "area": "Example: product delivery",
                "status": "Example: on track",
                "summary": "Example: the team completed a planned onboarding improvement.",
                "items": ["Example: released a clearer first-run workflow."],
            }
        ],
        "risks_blockers": [
            {
                "risk": "Example: release timing",
                "detail": "Example: a dependency could move the illustrative delivery date.",
                "severity": "Example: low severity",
            }
        ],
        "asks_decisions": [
            {
                "ask": "Example: confirm the next reporting priority.",
                "context": "Example: this keeps the illustrative team focused on the highest-value outcome.",
                "owner": "Example: CEO",
            }
        ],
        "next": ["Example: validate the onboarding flow with a new workspace."],
        "metrics": {"Example: pull requests merged": 4, "Example: deployments": 2},
        "sources": [],
    }
...
def locate_prior_report(json_path: Path, data: dict[str, Any]) -> tuple[Path | None, dict[str, Any] | None, str, list[str]]:
    ...
    for path in sorted(json_path.parent.glob("*.json")):
        ...
        if not isinstance(cand, dict):
            ...
            continue
        # A sample report is illustrative, not evidence. This exclusion cannot be inherited
        # from report_type: unknown types are coerced to ad_hoc below, so a sample would
        # otherwise become the baseline the first REAL report is diffed against. Silent by
        # design — a sample in this folder is expected, not an anomaly worth a stderr note.
...
.report-sample-card { border-color: var(--accent); border-style: dashed; }
.report-primary {
...
.report-sample strong { color: var(--accent); font-size: 11px; font-weight: 850; text-transform: uppercase; white-space: nowrap; }
.report-changes {
  margin: 0 0 10px;
  border: 1px solid var(--line);
  border-left: 5px solid var(--accent);
...
@media (max-width: 860px) {
  ...
  .report-sample { align-items: flex-start; flex-direction: column; gap: 3px; }
...
def write_sample_report(wiki_root: Path, company: str, stable_title: str, *, today: dt.date | None = None) -> Path | None:
    reports_dir = wiki_root / "reports" / "ceo-updates"
    reports_dir.mkdir(parents=True, exist_ok=True)
    report_path = reports_dir / f"{SAMPLE_REPORT_STEM}.html"
    if report_path.exists():
        return None

    has_real_html = any(path.stem != SAMPLE_REPORT_STEM for path in reports_dir.glob("*.html"))
    has_real_json = any(
        path.name != "data.json" and path.stem != SAMPLE_REPORT_STEM
        for path in reports_dir.glob("*.json")
    )
    if has_real_html or has_real_json:
        return None

    json_path = reports_dir / f"{SAMPLE_REPORT_STEM}.json"
    structured_data = build_sample_report_data(company, today)
    ...
    report_date = text_value(window.get("end")) if isinstance(window, dict) else ""
    body = render_structured_report("ceo-updates", structured_data, previous_data=None)
    relative_path = report_path.relative_to(wiki_root).as_posix()
    provenance = provenance_payload(
        wiki_root,
        artifact_id=f"ceo-updates:{SAMPLE_REPORT_STEM}",
        artifact_kind="ceo-updates",
        relative_path=relative_path,
        title=title,
        generated_at=utc_now(),
        extra={"reportDate": report_date, "sample": True},
    )
    report_path.write_text(
        render_report_page(
            title,
            ...
            provenance,
            stable_title,
            lede=report_lead_summary(structured_data),
        ),
        encoding="utf-8",
    )
...
def render_index(wiki_root: Path, project_folder: Path, today: dt.date | None = None) -> None:
    ...
    sample_links = []
    report_links = []
    for path in sorted((reports_dir / report_folder).glob("*.html"), reverse=True):
        (sample_links if is_sample_report_path(path) else report_links).append(path)
    ...
    sample_items = []
    sample_summary = "This example was generated by init. It is not counted as a report, is not part of the weekly streak, and is not used for comparison."
    for path in sample_links:
        href = path.relative_to(wiki_root).as_posix()
        title = html_title(path)
        date = "Example only"
        sample_items.append(
            f"""<a class="report report-sample-card" href="{esc(href)}" data-search-text="{search_text_attr("sample", title, date, sample_summary)}">
  <div class="rp-top">
    <div class="rp-head"><span class="rp-role">Sample report</span><span class="rp-name">{esc(title)}</span><span class="rp-date">{esc(date)}</span></div>
    <span class="rp-open-top">Open</span>
  </div>
  <p class="rp-prev">{esc(sample_summary)}</p>
</a>"""
        )
  ...
  <p class="rp-prev">{esc(summary)}</p>
        ...
        report_items.extend(sample_items)
        reports_html = f'<div class="reports reports-supporting">{"".join(report_items)}</div>'
    else:
        empty_state_html = f"""<article class="report report-primary empty" data-search-text="No CEO reports generated yet">
  <div class="rp-top">
  ...
  <p class="rp-purpose">Run /dzcto-ceo-report-weekly for the default weekly window, or /dzcto-ceo-report for a custom date range.</p>
        ...
        reports_html = (
            f'<div class="reports reports-supporting">{empty_state_html}{"".join(sample_items)}</div>'
            if sample_items
            else empty_state_html
        )
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--no-sample-report", action="store_true", help="Do not generate the first-run sample CEO report during init")
    ...
    if args.init:
        ...
        if not args.no_sample_report:
            sample_report = write_sample_report(wiki_root, company, stable_title)
            if sample_report:
                print(f"dzcto: sample CEO report generated at {sample_report}", file=sys.stderr)
